...
//   controlStatusLed();
	...
	if(recievedData == true){
		...
		if ( remPackage.type == NORMAL ) {
			...
			if ( remPackage.trigger ) {
			esc_speed = map(remPackage.throttle, 0, 1023, 0, 180);
			if (last_esc_speed == 0 and esc_speed != 0) {
				for ( int i = 1; i < 11; i = i+1 ) {

					esc.write(esc_speed/10 * i);
					delay(400);
				}
				last_esc_speed = esc_speed;
			} else {
				esc.write(esc_speed);
			}
