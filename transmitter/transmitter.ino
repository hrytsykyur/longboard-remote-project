...
const short rules[numOfSettings][3] {
	...
	{34, 0, 250},	// Wheel pulley
	{83, 0, 250},	// Wheel diameter
	{1, 0, 2}, 		// 0: PPM only   | 1: PPM and UART | 2: UART only
	{391, 0, 500},	// Min hall value
	{477, 300, 700},	// Center hall value
	{556, 500, 1023},	// Max hall value
	{-1, 0, 0}, 	  // Address
...
void calculateRatios() {
	// ERPM to Km/h
	ratioRpmSpeed = (60 * (float)txSettings.wheelDiameter * 3.14156) / (1000000); 
	// Pulses to km travelled
	ratioPulseDistance = ((float)txSettings.wheelDiameter * 3.14156) / (1000000); 
}
...
bool triggerActive() {
	if (digitalRead(triggerPin) == LOW) {
		return true;
		//dada
		// Serial.println("trigger pin at low");
	} else {
		return false;
}
}
...
float batteryPackPercentage( float voltage ){
	...
	float percentage = (100 - ( (maxCellVoltage * txSettings.batteryCells - voltage)/((maxCellVoltage - minCellVoltage) * txSettings.batteryCells) ) * 100);
